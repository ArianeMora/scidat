...
Setup the environment for using the scidat package in R. Here we need to do the following:
  ...
  4. Once that has installed, run the following: `source activate example`
  5. Run `pip install scidat`, this will install all the packages we'll need!
  3. Lastly, type: `where python` and copy that path into the `use_python("~/opt/miniconda3/envs/ml/bin/python")` below.
...
use_python("~/opt/miniconda3/envs/example/bin/python")
...
project_dir <- '~/Documents/TCGA_data_download_scidat/'
