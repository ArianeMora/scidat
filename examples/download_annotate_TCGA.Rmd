...
Setup the environment for using the scidat package in R. Here we need to do the following:
  ...
  3. Once conda is installed go to your terminal and type: `conda create -n pyenv python=3.8`, this will create a new `environment`
  4. Once that has installed, run the following: `source activate pyenv`
  5. Run `pip install scidat`, this will install all the packages we'll need!
  3. Lastly, type: `where python` and copy that path into the `use_python("/Users/ariane/opt/miniconda3/envs/ml/bin/python")` below.
...
use_python("/Users/ariane/opt/miniconda3/envs/ml/bin/python")
...
#download_dir <- '~/Documents/TCGA_data_download_scidat/'
project_dir <- '/Users/ariane/Documents/TCGA_data_download_scidat/'

# Files downloaded from TCGA
manifest_file <- paste(project_dir, 'manifest.tsv', sep='')
gdc_client <- paste(project_dir, './gdc-client', sep='')
...
gene_annotation_file <- paste(project_dir, "gene_info_20200416.tsv", sep="") # --> Use this if you haven't already created it: api$u$generate_label(c(project_dir, "gene_info"), ".tsv")
...
genes_of_interest <- c('FH', 'SDHB', 'SDHA', 'SDHC', 'SDHD', 'MET', 'ATF4', 'CHAC1', 'PSAT1', 'NRF2', 'SLC7A11', 'CDKN1A')
...
boxplot <- sciviso$Violinplot(box_df, "Conditions", "Values", "FH MUT and MET MUT", add_dots=TRUE)
boxplot$plot()
mpl$pyplot$show()
